...
public class PeopleController {
	...
	@RequestMapping(value="/getPeopleJSON.do", 
		//	method=RequestMethod.POST , 굳이 할필요 없다
	...
	@RequestMapping(value="/insertPeople.do", 
			method=RequestMethod.POST, //반드시 필요하다, 생략하면 겟포스트 다 처리한다.
...
//			produces="application/json;charset=utf-8")  //에이젝스와 타입동일해야 한다
//	@ResponseBody 
//	public Map<String, Object>  getPeople(String id){  //삽입
//		Map<String, Object>  retVal = new HashMap<String, Object>();//리턴값지정 //Object : 다양한 타입을 줄 수 있어서 사용
//		try {
//			peopleService.getPeople(id);//확인여부를 처리
//			retVal.put("res", "OK");
//		}
//		catch(Exception e) {
//			System.out.println("에러 : "+ e.getMessage());
...
//	}
	...
	@RequestMapping(value="/updatePeople.do", 	method=RequestMethod.POST,
			produces="application/json;charset=utf-8")  //에이젝스와 타입동일해야 한다
	@ResponseBody 
	public Map<String, Object>  updatePeople(PeopleVO vo){  //삽입
		Map<String, Object>  retVal = new HashMap<String, Object>();//리턴값지정 //Object : 다양한 타입을 줄 수 있어서 사용
		try {
			peopleService.updatePeople(vo);//확인여부를 처리
			retVal.put("res", "OK");
		}
		catch(Exception e) {
			System.out.println("에러 : "+ e.getMessage());
