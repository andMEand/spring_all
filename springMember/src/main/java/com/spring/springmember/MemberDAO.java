...
public class MemberDAO {
	...
	public int userCheck(MemberVO memberVO) {
		...
		try {
			...
			JDBCUtil.closeResource(pstmt, conn,rs);
		}
		return x;
	}
		...
		public MemberVO selectMember(MemberVO memberVO) { //회원정보 조회
			MemberVO vo = null;
		...
		public int updateMember (MemberVO memberVO) {//memberVO에 수정한 데이터를 전달
			int res = 0;
			...
			try {
				conn = JDBCUtil.getConnection();
				  ...
				  res=pstmt.executeUpdate();//result업뎃이 잘됐는지 확인하기 위해
			}
			catch(Exception ex) {
				ex.printStackTrace();
			}
